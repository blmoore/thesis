...
Of those variables with a non-zero coefficient at the optimally-selected tuning parameter $\lambda$ (Methods \ref{meth:lasso}), the ten largest in each cell type are shown (Fig. \ref{fig:lassobar}). Similarities can be observed with variable importance from subset models (Fig. \ref{fig:top10venn}), including the large (negative) coefficient for EGR1 in the H1 hESC model as well as that of P300 in K562 (Fig. \ref{fig:lassobar}).

Of interest is the appearance of group of factors known to interact as part of the activator protein-1 (AP-1), these include c-Fos, c-Jun and ATF1--3; all are spread across the most highly-ranked variables in each model of chromatin organisation (Fig. \ref{fig:lassobar}). The AP-1 complex has been shown to have DNA bending properties,\cite{Kerppola1993} and recently FOS and JUN members were associated with long-range chromatin interactions\cite{Heidari2014} suggesting an under-explored role for this complex in genome organisation.
