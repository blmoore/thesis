...
consortia, most notably the ENCODE\cite{Dunham2012} and NIH
...
We can extend this approach to the related domain of nuclear architecture, in the hope of understanding the relationships between chromatin and higher order structure in the same way that chromatin features have been related to transcriptional output.

\section{Reproducing Dong \emph{et al.} }\label{sec:reprodong}

Following \citet{Dong2012}, we first
reimplemented the published ENCODE modelling framework to 
replicate their results. In doing so we were able to analyse the
strengths and caveats of their approach; for example we found, surprisingly, the two-step classification
...
\caption[Comparison of two-step classification-regression model of transcription with
  ...
  Two-step model $0.872 \pm 9.89 \times 10^{-5} $.
...
Additionally, the $35$ variables used thus far as model inputs are not the complete set available in each cell type, but only the subset of those assayed in all three cell types under study. The ENCODE consortium has produced a significantly greater number of datasets\cite{Dunham2012, Boyle2014} in each cell type which have thus far gone unused. In this section we'll explore models of higher order chromatin structure, in some cases built from over $100$ variables, and then also generate parsimonious models using optimal subsets guided by statistical techniques that penalising model complexity.

\subsection{Full models}

% Compare RF with previous

\subsection{BIC-penalised multiple linear regression}
i
...
\multirow{2}{*}{} & \multicolumn{3}{c}{GM$12878$} & \multicolumn{3}{c}{H$1$ hESC} & \multicolumn{3}{c}{K$562$} \\ \cline{2-10} 
 & \multicolumn{1}{|c|}{n} & \multicolumn{1}{c}{LM} & \multicolumn{1}{c|}{RF} & \multicolumn{1}{c|}{n} & \multicolumn{1}{c}{LM} & \multicolumn{1}{c|}{RF} & \multicolumn{1}{c|}{n} & \multicolumn{1}{c}{LM} & \multicolumn{1}{c|}{RF} \\ \hline
\multicolumn{1}{|l|}{All features} & 115 & .836 & \multicolumn{1}{l|}{.828} & 71 & .744 & \multicolumn{1}{l|}{.755} & 187 & .811 & \multicolumn{1}{l|}{.813} \\
\multicolumn{1}{|l|}{Optimal subset} & 21 & .840 & \multicolumn{1}{l|}{XXX} & 28 & .746 & \multicolumn{1}{l|}{XXX} & 37 & .819 & \multicolumn{1}{l|}{XXX} \\
\multicolumn{1}{|l|}{Matched subset} & 35 & .827 & \multicolumn{1}{l|}{.823} & 35 & .740 & \multicolumn{1}{l|}{.747} & 35 & .796 & \multicolumn{1}{l|}{.799} \\ \hline
\end{tabular}
}
\end{table}
...
\begin{center} 
\makebox[\textwidth][c]{ 
\includegraphics[width=1.2\textwidth]{figs/corcirc.pdf}
}
