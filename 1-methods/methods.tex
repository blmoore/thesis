...
Lamina associated domains (LADs) are genomic regions which are in contact lamin proteins A, B and C, found on the inner nuclear membrane (reviewed in \citenum{Luperchio2014}). To compare metaTAD boundaries with those of LADs, we made use of previously-published Lamin-B1 DamID microarray probe intensities.\cite{Peric-Hupkes2010} For analysis over boundaries, these values were averaged into the same boundary windows as used previously (50 kb bins $\pm450$ kb around boundary, as in Section \ref{meth:tadbounds}). 
