...
\citet{Dixon2012} using their recommended parameters. This method is introduced in Section \ref{intro:tads}.  % XX write DI/ tad calling highlights

The TAD calling algorithm is a multi-stage process. Firstly, XX

For the generation
of boundary profiles, input features were
averaged into 50 kb bins spanning $\pm450$ kb from the boundary bin.
...
MetaTADs are a concept discovered by collaborators. Their method for calling such features involve the constrained hierarchical cluttering of neighbouring TADs with the greatest inter-TAD contacts. This results in a tree of increasing metaTAD aggregation. % expand XX
